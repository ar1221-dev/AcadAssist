...
def test_settings_defaults():
    ...
    assert settings.foundry_model_deployment == "gpt-4o"
    assert settings.foundry_embedding_deployment == "text-embedding-3-small"
    assert settings.azure_search_index == "acadassist-index"
    assert settings.azure_storage_container == "study-materials"
