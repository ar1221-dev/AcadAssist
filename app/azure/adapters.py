...
class KnowledgeBaseAdapter:
    ...
    def search_knowledge(
        ...
        logger.info("Executing search_knowledge for user=%s query='%s'", user_id, query)
    ...
    def summarize_document(self, user_id: str, document_id: str, mode: str = "concise") -> Dict[str, Any]:
...
class AssessmentAdapter:
    ...
    def generate_quiz(
        ...
        logger.info(
            "Generating quiz for user=%s, subject=%s, topics=%s, difficulty=%s, count=%s",
            user_id,
            subject_id,
            topic_ids,
            difficulty,
        ...
        # Filter out generic syllabus fallback chunks if no specific material was found
        valid_chunks = [c for c in kb_resp.results if c.chunk_id != "chunk-gen-001"]

        # If no relevant material exists, return insufficient material response per requirement 7
        if not valid_chunks:
            logger.info("Insufficient study material found for query '%s'", query_str)
            return {
                "quiz_id": None,
                "subject_id": subject_id,
                "topic_ids": topic_ids,
        ...
        elif any(term in q_lower for term in ["paging", "page", "virtual memory", "tlb", "frame"]) or any(
...
class ToolDispatcher:
    ...
    def dispatch(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns a dictionary result containing output and optionally extracted sources and action details.
        """
        ...
        if tool_name == "search_knowledge":
            ...
            res = self.study_adapter.generate_weekly_report(
                user_id=arguments.get("user_id", ""),
                week_start=arguments.get("week_start", "2026-09-14"),
                week_end=arguments.get("week_end", "2026-09-20"),
            )
