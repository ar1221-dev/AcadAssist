...
AcadAssist includes unified health check and lightweight Azure diagnostic endpoints that do not require the Azure CLI:

* `GET /api/health` (also accessible at `/health`): Unified application health status, database ping, and subsystem readiness (Knowledge Base, Assessment, Study Intelligence).
* `GET /api/health/azure`: Live read-only connectivity diagnostics for Azure Blob Storage, Azure AI Search, and Microsoft Foundry. Returns one of:
  * `not_configured`: Service credentials or endpoints are omitted (local mode active).
  * `connected`: Lightweight read-only check confirmed service communication.
  * `authentication_failed`: Invalid credentials, expired tokens, or Entra ID rejection.
  * `unreachable`: Network error, DNS resolution failure, or endpoint timeout.
  * `error`: Unexpected service error response.

> **Zero Secret Leakage:** Diagnostic responses sanitize all internal exceptions and NEVER return API keys, connection strings, passwords, or authentication headers.
> **Frontend Integration:** Service status is accessible via the **Settings -> System Diagnostics & Cloud Status** panel in the React UI with real-time refresh.
