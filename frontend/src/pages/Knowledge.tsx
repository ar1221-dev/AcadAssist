...
export default function Knowledge() {
  ...
  const processedCount = knowledgeMaterials.filter(m => m.status === 'Ready').length;
  ...
  const askAi = (m: Material) => {
    navigate(
      `/assistant?prompt=${encodeURIComponent(
        `Explain the key principles and potential exam questions from ${m.name}`
      )}&material=${encodeURIComponent(m.id || '')}`
    );
  };
