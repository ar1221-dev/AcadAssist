...
import { Download, Shield, Lock, UserRound, Plus, Pencil, Trash2, Moon, Sun, Monitor, Check, RefreshCw, CheckCircle2, AlertCircle, XCircle, Cloud } from 'lucide-react';
import HeroHeader from '../components/ui/HeroHeader';
import SectionHeader from '../components/ui/SectionHeader';
import Modal from '../components/ui/Modal';
import { useApp } from '../context/AppContext';
import { fetchSystemHealth, fetchAzureDiagnostics, type SystemHealthInfo, type AzureDiagnosticsInfo } from '../services/api';
...
export default function Settings() {
  ...
  const [azureHealth, setAzureHealth] = useState<AzureDiagnosticsInfo | null>(null);
  const [checkingHealth, setCheckingHealth] = useState(false);

  const checkDiagnostics = useCallback(async () => {
    setCheckingHealth(true);
    try {
      const [sys, az] = await Promise.all([
        fetchSystemHealth(),
        fetchAzureDiagnostics(),
      ]);
      setSystemHealth(sys);
      setAzureHealth(az);
    } catch {
  ...
  const renderDiagnosticBadge = (status?: string, loading?: boolean) => {
    if (loading) {
      return (
        <span className="soft-badge text-gray-600 bg-gray-100 flex items-center gap-1 font-bold">
    ...
    if (status === 'connected') {
      return (
        <span className="soft-badge text-emerald-700 bg-emerald-50 dark:bg-emerald-950/40 dark:text-emerald-300 flex items-center gap-1 font-bold">
    ...
    if (status === 'not_configured') {
      return (
        <span className="soft-badge text-amber-700 bg-amber-50 dark:bg-amber-950/40 dark:text-amber-300 flex items-center gap-1 font-bold">
    ...
    if (status === 'authentication_failed') {
      return (
        <span className="soft-badge text-red-700 bg-red-50 dark:bg-red-950/40 dark:text-red-300 flex items-center gap-1 font-bold">
          <XCircle size={12} /> Auth Failed
        </span>
      );
    }
    ...
    return (
      <span className="soft-badge text-red-700 bg-red-50 dark:bg-red-950/40 dark:text-red-300 flex items-center gap-1 font-bold">
        <XCircle size={12} /> {status || 'Unknown'}
      </span>
    );
  };
    ...
    <div className="page-stack">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <SectionHeader icon="☁️" title="System Diagnostics & Cloud Status" />
          <div className="card p-4 mb-6">
            <div className="flex justify-between items-center mb-3">
              <div>
                <div className="text-xs font-bold text-[var(--color-text-dark)]">System & Cloud Connectivity</div>
                <div className="text-[.68rem] text-[var(--color-text-muted)]">Live service diagnostic without CLI dependencies</div>
              </div>
              <button
                className="btn subtle text-xs py-1 px-2.5"
            ...
            <div className="flex flex-col gap-2 pt-1 border-t border-[var(--color-border-light)] text-xs">
              <div className="flex justify-between items-center py-1.5 border-b border-[var(--color-border-light)]">
                <span className="font-medium text-[var(--color-text-body)]">Backend API</span>
                <span className={`soft-badge font-bold flex items-center gap-1 ${systemHealth ? 'text-emerald-700 bg-emerald-50 dark:bg-emerald-950/40 dark:text-emerald-300' : 'text-red-700 bg-red-50 dark:bg-red-950/40 dark:text-red-300'}`}>
                  {systemHealth ? <CheckCircle2 size={12} /> : <XCircle size={12} />}
                  {systemHealth ? 'Connected' : 'Unavailable'}
                </span>
              </div>

              <div className="flex justify-between items-center py-1.5 border-b border-[var(--color-border-light)]">
                <span className="font-medium text-[var(--color-text-body)]">Database</span>
                <span className={`soft-badge font-bold flex items-center gap-1 ${systemHealth?.database === 'connected' ? 'text-emerald-700 bg-emerald-50 dark:bg-emerald-950/40 dark:text-emerald-300' : 'text-red-700 bg-red-50 dark:bg-red-950/40 dark:text-red-300'}`}>
                  {systemHealth?.database === 'connected' ? <CheckCircle2 size={12} /> : <XCircle size={12} />}
                  {systemHealth?.database === 'connected' ? 'Connected' : 'Unreachable'}
                </span>
              </div>

              <div className="flex justify-between items-center py-1.5 border-b border-[var(--color-border-light)]">
                <span className="font-medium text-[var(--color-text-body)]">Azure Blob Storage</span>
                {renderDiagnosticBadge(azureHealth?.services?.azure_blob_storage?.status, checkingHealth)}
              </div>

              <div className="flex justify-between items-center py-1.5 border-b border-[var(--color-border-light)]">
                <span className="font-medium text-[var(--color-text-body)]">Azure AI Search</span>
                {renderDiagnosticBadge(azureHealth?.services?.azure_ai_search?.status, checkingHealth)}
              </div>

              <div className="flex justify-between items-center py-1.5">
                <span className="font-medium text-[var(--color-text-body)]">Microsoft Foundry</span>
                {renderDiagnosticBadge(azureHealth?.services?.microsoft_foundry?.status, checkingHealth)}
              </div>
            </div>

            {azureHealth?.message && (
              <div className="mt-3 p-2.5 rounded-lg bg-[var(--color-bg-subtle)] text-[.66rem] text-[var(--color-text-muted)] flex items-start gap-1.5">
                <Cloud size={14} className="shrink-0 mt-0.5" />
                <span>{azureHealth.message}</span>
              </div>
            )}
          </div>
