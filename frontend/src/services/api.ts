  ...
  return {
    ...
    questions: res.questions.map(q => {
      ...
      if (q.correct_answer && Array.isArray(q.options)) {
        const caClean = q.correct_answer.trim().toLowerCase();
        const foundIdx = q.options.findIndex(opt => {
          const optClean = opt.trim().toLowerCase();
          return (
            optClean === caClean ||
            (caClean.length >= 2 && optClean.startsWith(caClean.slice(0, 2))) ||
            (optClean.length >= 2 && caClean.startsWith(optClean.slice(0, 2))) ||
            (caClean.length > 3 && optClean.includes(caClean)) ||
            (optClean.length > 3 && caClean.includes(optClean))
          );
        });
        if (foundIdx !== -1) {
...
// Health and Azure Diagnostics API Endpoints (/api/health and /api/health/azure)
export interface SystemHealthInfo {
  status: 'healthy' | 'degraded' | string;
  service: string;
  environment: string;
  version: string;
  database: 'connected' | 'unreachable' | string;
  subsystems: {
    knowledge_rag: string;
    assessment: string;
    study_intelligence: string;
  };
}

export interface AzureServiceDiagnostic {
  status: 'connected' | 'not_configured' | 'authentication_failed' | 'unreachable' | 'error' | 'not_implemented';
  message: string;
}

export interface AzureDiagnosticsInfo {
  status: 'connected' | 'not_configured' | 'authentication_failed' | 'unreachable' | 'error' | 'not_implemented';
  services: {
    azure_blob_storage: AzureServiceDiagnostic;
    azure_ai_search: AzureServiceDiagnostic;
    microsoft_foundry: AzureServiceDiagnostic;
  };
  message: string;
}

export async function fetchSystemHealth(): Promise<SystemHealthInfo | null> {
  try {
    return await request<SystemHealthInfo>('/api/health');
  } catch {
    return null;
  }
}

export async function fetchAzureDiagnostics(): Promise<AzureDiagnosticsInfo | null> {
  try {
    return await request<AzureDiagnosticsInfo>('/api/health/azure');
  } catch {
    return null;
  }
}
